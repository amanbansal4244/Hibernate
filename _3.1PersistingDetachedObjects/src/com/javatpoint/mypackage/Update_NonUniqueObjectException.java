 ...
 */
public class Update_NonUniqueObjectException {
...

	{
		...
		p.setPrice("555555");            // p is in Detached state
		
		Session session2=factory.openSession();
		/****
		 * When we call update() method, if already a session cache containing the same object
		 * then the update() method throws an exception called NonUniqueObjectException.
		 * Because Hibernate can’t decide which object represents the current state.
		 */
		Object o2=session2.get(Product.class, new Integer(1));
